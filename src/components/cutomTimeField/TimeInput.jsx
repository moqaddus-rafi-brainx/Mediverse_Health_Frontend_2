...
export default class TimeField extends React.Component {
  ...
  onInputChange(event, callback) {
    ...
    if (addedCharacter !== null) {
      if (position > this.state._maxLength) {
        ...
        // user typed a number
        newValue =
          inputValue.substr(0, position - 1) + addedCharacter + inputValue.substr(position + 1);
  ...
  render() {
    ...
    const { onChange, onBlur, style, showSeconds, input, inputRef, className, colon, ...props } =
      this.props; //eslint-disable-line no-unused-vars
    const onChangeHandler = (event) =>
      this.onInputChange(event, (e, v) => onChange && onChange(e, v));

    const onBlurHandler = (event) => this.onInputChange(event, (e, v) => onBlur && onBlur(e, v));
